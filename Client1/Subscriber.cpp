...
struct pubsub_opts opts =
{
	...
	MQTTVERSION_DEFAULT, NULL, "Client3main", 0, 0, NULL, NULL, "localhost", "1883", NULL, 10, /* MQTT options */
...
int sub_messageArrived(void *context, char *topicName, int topicLen, MQTTAsync_message *message)
{
	size_t delimlen = 0;
...
void sub_onConnectFailure5(void* context, MQTTAsync_failureData5* response)
{
	if (!opts.quiet)
...
void sub_onConnect5(void* context, MQTTAsync_successData5* response)
{
...
void sub_trace_callback(enum MQTTASYNC_TRACE_LEVELS level, char* message)
{
...
int subscribe(MQTTAsync client, MQTTAsync_createOptions create_opts, char* url)
{
	...
	{
		url = (char*)malloc(100);
		sprintf_s(url, 500,"%s:%s", opts.host, opts.port);
	}
	if (opts.verbose)
		printf("URL is %s\n", url);
	...
	{
		MQTTAsync_setTraceCallback(sub_trace_callback);
	...
	rc = MQTTAsync_setCallbacks(client, client, NULL, sub_messageArrived, NULL);
	if (rc != MQTTASYNC_SUCCESS)
	{
		if (!opts.quiet)
...
#if defined(_WIN32)
	signal(SIGINT, sub_cfinish);
	signal(SIGTERM, sub_cfinish);
#else
	...
	if (opts.MQTTVersion == MQTTVERSION_5)
	{
		...
		conn_opts.onSuccess5 = sub_onConnect5;
		conn_opts.onFailure5 = sub_onConnectFailure5;
